...
#### ![](./assets/c9b9b4b0eba843cfba5b97215d76d98b-20240619141915260.png)
...
![](./assets/ad21423c5e3f47689551b2e3973d8304-20240619141915263.png)





ForEach接口基于数组类型数据来进行循环渲染，需要与容器组件配合使用，且接口返回的组件应当是允许包含在ForEach父容器组件中的子组件。例如，ListItem组件要求ForEach的父容器组件必须为[List组件](https://docs.openharmony.cn/pages/v4.1/zh-cn/application-dev/reference/apis-arkui/arkui-ts/ts-container-list.md "List组件")。
...
| 参数名 | 参数类型   | 是否必填 | 参数描述                      |
| ------ | ---------- | -------- | ----------------------------- |
| arr    | Array<any> | 是       | 数据源，为`Array`类型的数组。 |
**说明：**  
\- 可以设置为空数组，此时不会创建子组件。  
...
![ForEach-Key-Generation-Rules](./assets/dfe1cc1474f65d83b3ff36eacfb0ed33.png)
...
在确定键值生成规则后，ForEach的第二个参数`itemGenerator`函数会根据键值生成规则为数据源的每个数组项创建组件。组件的创建包括两种情况：ForEach首次渲染和ForEach非首次渲染。
...
![ForEach-CaseStudy-1stRender-NoDup](./assets/a42c7e40280364458844789b35ecbe34.png)
...
![ForEach-CaseStudy-1stRender-Dup](./assets/409aa1cd5e696b52eb00f8be20da10a1.png)
...
![ForEach-Non-Initial-Render-Case-Effect](./assets/663bcc7aff85121d6077b77ec50c14f8.png)
...
![ForEach-SkeletonScreen](./assets/f05679c3c42de094821cdbbe04e0091a.png)
...
![ForEach-DataSourceArrayChange](./assets/1e36cd31dbbeb81ace75f325894b6b6f.png)
...
![ForEach-DataSourceArraySubpropertyChange](./assets/419bd0892fa8c6c96c12afbd954da116.png)
...
![ForEach-UnexpectedRenderingResult](./assets/94f5b43797254476caa3b968ecc1c3c6.png)
...
在本示例中，`ForEach`的第三个参数`KeyGenerator`函数处于缺省状态。根据上述键值生成规则，此例使用框架默认的键值生成规则，即最终键值为字符串`index + '__' + JSON.stringify(item)`。当点击“在第1项后插入新项”文本组件后，`ForEach`将需要为第2个数组项以及其后的所有项重新创建组件。
...
![ForEach-RenderPerformanceDecrease](./assets/ed4f9d9c7b65e586de6bc7dbed56b773.png)

点击“在第1项后插入新项”文本组件后，IDE的日志打印结果如下所示。

**图10** 渲染性能降低案例日志打印图

![ForEach-RenderPerformanceDecreaseLogs](./assets/3861b9e5b1a4646c1935e0cb051190ee.png)

插入新项后，`ForEach`为`new item`、 `two`、 `three`三个数组项创建了对应的组件`ChildItem`，并执行了组件的[aboutToAppear()](https://docs.openharmony.cn/pages/v4.1/zh-cn/application-dev/reference/apis-arkui/arkui-ts/ts-custom-component-lifecycle.md#abouttoappear "aboutToAppear()")生命周期函数。这是因为：
